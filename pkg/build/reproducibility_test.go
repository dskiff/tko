...
func TestReproducibleBuild_TarLayerTimestamps(t *testing.T) {
	...
	for {
		...
		// AccessTime and ChangeTime are set to epoch in production code for
		// deterministic TAR bytes, but USTAR format only stores ModTime.
		// When present (PAX format), verify they are epoch.
		if !header.AccessTime.IsZero() && !header.AccessTime.Equal(unixEpoch) {
			t.Fatalf("entry %q: AccessTime = %v, want unix epoch or zero", header.Name, header.AccessTime)
		}
		if !header.ChangeTime.IsZero() && !header.ChangeTime.Equal(unixEpoch) {
			t.Fatalf("entry %q: ChangeTime = %v, want unix epoch or zero", header.Name, header.ChangeTime)
		}
		// PAXRecords may be auto-generated by the tar writer for atime/ctime;
		// the key guarantee is that they contain only deterministic values.
		// Verify no SCHILY.xattr entries leaked through.
		for k := range header.PAXRecords {
			if strings.HasPrefix(k, "SCHILY.xattr.") {
				t.Fatalf("entry %q: unexpected xattr PAX record: %s", header.Name, k)
			}
		}
